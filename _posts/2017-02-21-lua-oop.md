...
function ClassObject(obj) --创建函数, 传入一个 obj

    if obj == nil then
        obj = {}
    end
    return setmetatable(obj, {__index=Class}) --设定这个 obj 的 metatable 的 __index 为 Class
end
```

如上, 当我们调用的时候, 返回一个 `metatable` 为 `Class` 的 `table`.
